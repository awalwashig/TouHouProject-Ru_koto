...
void voice::voice_link(const dpp::slashcommand_t& event)
{
...
void voice::voice_break_link(const dpp::slashcommand_t& event)
{
	if (event.command.get_command_name() == "断开雅语交合") {
		//dpp::find_guild(event.command.guild_id)->connect_member_voice(event.command.get_issuing_user().id);

		event.reply(dpp::message("断开语音连接ing").set_flags(dpp::m_ephemeral));
	}
}

void voice::on_slashcommand_voice(const dpp::slashcommand_t& event)
{
